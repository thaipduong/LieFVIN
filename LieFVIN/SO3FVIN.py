...
class SO3FVIN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.enable_grad():
            ...
            V_qk = self.V_net(qk)
            dVk =  torch.autograd.grad(V_qk.sum(), qk, create_graph=True)[0]
            dVk = dVk.view(-1, 3, 3)
            ...
            dVk_next =  torch.autograd.grad(V_qk_next.sum(), qk_next, create_graph=True)[0]
            ...
            pk_next = torch.matmul(FkT, pk[:,:,None]) + (1-alpha)*self.h*torch.matmul(FkT, Mk[:,:,None]) + \
                      alpha*self.h*Mk_next[:,:,None] + torch.matmul(FkT, fk_minus[:,:,None]) + fk_plus[:,:,None]
