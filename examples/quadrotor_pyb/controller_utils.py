...
def stateToQd(x):
    ...
    #print("Rot:\n", Rot)
    print("rotmat in my qd struct:", qd.Rot.flatten())
    print("pos in my qd struct:", qd.pos, "vel ", qd.vel)
    [phi, theta, yaw] = RotToRPY_ZXY(qd.Rot)
    qd.euler = np.array([phi, theta, yaw])
    qd.omega = x[10:13]
    return qd

def stateToQdRotMat(x):
    """
    Converts qd struct used in hardware to x vector used in simulation
    x is 1 x 13 vector of state variables [pos vel quat omega]
    qd is a struct including the fields pos, vel, euler, and omega
    ...
    #print("Rot:\n", Rot)
    print("rotmat in my qd struct:", qd.Rot.flatten())
    print("pos in my qd struct:", qd.pos)
    [phi, theta, yaw] = RotToRPY_ZXY(qd.Rot)
    qd.euler = np.array([phi, theta, yaw])
    qd.omega = x[10:13]
    return qd
...
def dummy_ref_fa(s, t):
    """
    Desired diamond trajectory
    """
    pos = s[0:3]
    pos_des = np.array([2, 0.0, 3.0])
    # vel_des = np.array([0, 0, 0])
    # acc_des = np.array([0, 0, 0])

    #pos_des = np.array([21.0895,  3.5726,  1.0000])
    vel_des = np.array([0, 0, 0])
    acc_des = np.array([0, 0, 0])
    ...
    # r2 = np.cross(e3, r1)
    # r3 = np.cross(r1, r2)
    # rotmat_des = np.vstack((r1, r2, r3)).T
    #rotmat_des = np.array([0.1354,  0.9908,  0.0000, -0.9908, 0.1354,  0.0000,  0.0000,  0.0000,  1.0000]).reshape((3,3))#np.eye(3)
    yaw_des = 0.0#3*np.pi/4
    rotmat_des = np.array([np.cos(yaw_des), -np.sin(yaw_des), 0.0000, np.sin(yaw_des), np.cos(yaw_des), 0.0000, 0.0000, 0.0000, 1.0000]).reshape(
        (3, 3))  # np.eye(3)
    print("Desired rot mat: ", rotmat_des.flatten())
    print("Desired pos: ", pos_des.flatten())
    ...
    desired_state.rotmat = rotmat_des
    desired_state.omega = omega_des
    desired_state.yaw_des = yaw_des
    return desired_state
...
def diamond(t):
    ...
    desired_state = state_object()
    desired_state.pos = scale*pos
    desired_state.vel = scale*vel
    desired_state.acc = scale*acc
    desired_state.yaw = yaw
    desired_state.yawdot = yawdot
...
def plot_states1D_pitch(s_traj, s_plan, fig_num=None):
    """
    Plot position and velocity with each X, Y, Z dimension on a separate axis
    """
...
def plot_traj1D(s_traj, fig_num=None, quad = True):
    """
    Plot position and velocity with each X, Y, Z dimension on a separate axis
    """

    plt.figure(fig_num, figsize=(10,10.5))
    ax_px = plt.subplot(521)
    ax_py = plt.subplot(523)
    ax_pz = plt.subplot(525)
    ax_roll = plt.subplot(522)
    ax_pitch = plt.subplot(524)
    ax_yaw = plt.subplot(526)

    ax_vxyz = plt.subplot(527)
    #ax_vy = plt.subplot(424)
    #ax_vz = plt.subplot(426)
    ax_w =  plt.subplot(528)
    ax_f = plt.subplot(529)
    ax_tau = plt.subplot(5,2,10)
    ...
    ax_pz.plot(s_traj[:, -1], s_traj[:, 2])
    #.plot(s_plan[:, -1], s_plan[:, 2])
    ax_pz.set_ylabel('z (m)')

    ax_vxyz.plot(s_traj[:, -1], s_traj[:, 6])
    ax_vxyz.plot(s_traj[:, -1], s_traj[:, 7])
    ax_vxyz.plot(s_traj[:, -1], s_traj[:, 8])
    #ax_vx.plot(s_plan[:, -1], s_plan[:, 3])
    ax_vxyz.set_ylabel('vx, vy, vz (m/s)')
    ...
    ax_roll.plot(s_traj[:, -1], s_traj[:, 3])
    #ax_yaw.plot(s_plan[:, -1], s_plan[:, 9])
    ax_roll.set_ylabel('roll (rad)')
    ax_pitch.plot(s_traj[:, -1], s_traj[:, 4])
    #ax_yaw.plot(s_plan[:, -1], s_plan[:, 9])
    ax_pitch.set_ylabel('pitch (rad)')
    ax_yaw.plot(s_traj[:, -1], s_traj[:, 5])
    #ax_yaw.plot(s_plan[:, -1], s_plan[:, 9])
    ax_yaw.set_ylabel('yaw (rad)')


    ax_w.plot(s_traj[:, -1], s_traj[:, 9])
    ax_w.plot(s_traj[:, -1], s_traj[:, 10])
    ax_w.plot(s_traj[:, -1], s_traj[:, 11])
    #ax_w.plot(s_traj[:, -1], 0*s_traj[:, -1])
    ax_w.set_ylabel(r'$\omega$ (rad/s)')
    ...
    if quad:
        ax_f.plot(s_traj[:, -1], s_traj[:, 12])
        # ax_w.plot(s_traj[:, -1], 0*s_traj[:, -1])
        ax_f.set_ylabel(r'force f')

        ax_tau.plot(s_traj[:, -1], s_traj[:, 13])
        ax_tau.plot(s_traj[:, -1], s_traj[:, 14])
        ax_tau.plot(s_traj[:, -1], s_traj[:, 15])
        # ax_w.plot(s_traj[:, -1], 0*s_traj[:, -1])
        ax_tau.set_ylabel(r'torque tau')
    else:
        ax_f.plot(s_traj[:, -1], s_traj[:, 12])
        ax_f.plot(s_traj[:, -1], s_traj[:, 13])
        ax_f.plot(s_traj[:, -1], s_traj[:, 14])
        # ax_w.plot(s_traj[:, -1], 0*s_traj[:, -1])
        ax_f.set_ylabel(r'force f')

        ax_tau.plot(s_traj[:, -1], s_traj[:, 15])
        ax_tau.plot(s_traj[:, -1], s_traj[:, 16])
        ax_tau.plot(s_traj[:, -1], s_traj[:, 17])
        # ax_w.plot(s_traj[:, -1], 0*s_traj[:, -1])
        ax_tau.set_ylabel(r'torque tau')

    #ax_px.set_title('Position/Lin. Vel.')
    #ax_yaw.set_title('rpy/omega')
