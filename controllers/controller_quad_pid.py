...
class QuadPIDControl():
    ...
    def __init__(self,
        ...
        self.GRAVITY = g*env.M
        self.P_COEFF_FOR = 0.3 * np.array([.4, .4, 1.25])
        self.I_COEFF_FOR = 0.3 * np.array([.05, .05, .05])
        self.D_COEFF_FOR = 0.3 * np.array([.2, .2, .5])
        self.P_COEFF_TOR = 0.3 * np.array([0.01, 0.01, 0.01])
        self.I_COEFF_TOR = 0.1 * np.array([.0, .0, 0.0001])
        self.D_COEFF_TOR = 0.05 * np.array([0.01, 0.01, 0.01])
    ...
    def computeControl(self,
        ...
        # thrust_torques = np.matmul(conversion_mat, self.KF * (rpm ** 2))
        return thrust, torques, pos_e # computed_target_rpy[2] - cur_rpy[2]
    ...
    def _dslPIDPositionControl(self,
        ...
        return scalar_thrust, target_euler, pos_e
