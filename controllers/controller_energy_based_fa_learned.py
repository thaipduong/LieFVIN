...
from se3hamneuralode import SE3FAHamNODE, from_pickle
...
class LearnedEnergyBasedControllerFA:
    ...
    def get_model(self):
        model = SE3FAHamNODE(device=device, pretrain=False).to(device)
        path = self.model_path + '/fadronesim-se3ham-rk4-5p3-8101.tar'
        model.load_state_dict(torch.load(path, map_location=device))
        path = self.model_path + '/fadronesim-se3ham-rk4-5p-stats.pkl'
