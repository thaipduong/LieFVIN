...
import numpy as np
import torch
from se3hamneuralode import SE3HamNODE_M1, from_pickle
...
class ControllerParams:
    def __init__(self):
        ...
        """
        self.maxangle = 45 * np.pi / 180  # you can specify the maximum commanded angle here
        self.K_p = 1*np.array([5, 5, 5])  # K_p
        self.K_v = 0.5*np.array([5, 5, 5])  # K_v
        self.K_R = 1 * np.array([250, 250, 250])  # K_R
        self.K_w = 1 * np.array([20, 20, 20])  # K_w

class LearnedEnergyBasedControllerQuad:
    def __init__(self, m, model_path = "data/run1/"):
    ...
    def get_model(self):
        model = SE3HamNODE_M1(device=device, pretrain=False).to(device)
    ...
    def gen_control(self, qd):
        ...
        # Calculate w_c
        b_p_dot = scale_acc * np.matmul(M1, K_p * (v - np.matmul(RT, v_ref)) + np.matmul(RT,
                                                                                         j_ref))  # + K_dv*(np.matmul(RT, a_ref)) \
        ...
        b2_dot = np.cross(
            np.cross(b2, (np.cross(b1_ref_dot, b3) + np.cross(b1_ref, b3_dot)) / np.linalg.norm(np.cross(b1_ref, b3))),
            b2)
