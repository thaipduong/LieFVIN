...
class PendulumEnvV1(gym.Env):
    ...
    def __init__(self, g=10.0, ori_rep = 'angle', friction=False, dt = 0.01):
        self.max_speed=100.
        self.max_torque=5.
        self.dt=dt
        self.g = g
        ...
        self.seed()
        print("Pendulum created with dt = ", dt)
    ...
    def step(self,u):
        ...
        ivp = solve_ivp(fun=lambda t, y:self.dynamics(t, y, u), t_span=[0, self.dt], y0=self.state, rtol=1e-10, atol=1e-10)
        self.state = ivp.y[:, -1]

        return self.get_obs(), -costs, False, {}

    def reset(self, ori_rep = 'angle', init_angle = None):
        high = np.array([np.pi, 1])
        if init_angle == None:
            self.state = self.np_random.uniform(low=-high, high=high)
        else:
            self.state = np.array([init_angle, 0], dtype=np.float32)
        self.last_u = None
        # Orientation representations: 'angle', 'rotmat'
        self.ori_rep = ori_rep
        return self.get_obs()
